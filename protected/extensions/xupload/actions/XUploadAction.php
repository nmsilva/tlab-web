    ...
    public function run( ) {
        ...
        if( isset( $_GET["_method"] ) ) {
            if( $_GET["_method"] == "delete" ) {
                ...
                $media= Media::model()->findByPk($_GET["file"]);
                
                $file=$this->path."/".$media->PATH;
                $success = is_file( $file) && unlink($file);
                
                MediaGaleria::model()->deleteAll('ID_MEDIA=:id', array('id'=>$media->ID_MEDIA));
                $media->delete();
            ...
            if( $model->file !== null ) {
                ...
                $name=$rnd."_";
                $model->name = "{$name}.{$model->file->getExtensionName()}";  // random number + file name
                ...
                if( $model->validate( ) ) {
                    ...
                    $media_model->DATA_CRIACAO = new CDbExpression('NOW()');
                    $media_model->NOME=$name;
                    $media_model->PATH=$this->_subfolder."/".$model->name;
                    $media_model->TYPE=0;
                    ...
                    if(isset($_GET['id_galeria']))
                    {
                        ...
                        $sec_model->ID_MEDIA=$media_model->ID_MEDIA;
                        $sec_model->ID_GALERIA=$_GET['id_galeria'];
